...
public final class BitcoinIntegration {
    ...
    private static final String MIMETYPE_PAYMENTREQUEST = "application/jadepay-paymentrequest"; // BIP 71
    ...
    private static Intent makeBitcoinUriIntent(final String address, final Long amount) {
        final StringBuilder uri = new StringBuilder("jadepay:");
        if (address != null)
            uri.append(address);
        if (amount != null)
            uri.append("?amount=")
    ...
    private static void redirectToDownload(final Context context) {
        Toast.makeText(context, "No Jadepay application found.\nPlease install Jadepay Wallet.", Toast.LENGTH_LONG)
                .show();
        ...
        final Intent binaryIntent = new Intent(Intent.ACTION_VIEW,
                Uri.parse("https://github.com/HashEngineering/jadepay-wallet/releases"));
