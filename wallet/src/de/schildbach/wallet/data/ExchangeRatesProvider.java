...
public class ExchangeRatesProvider extends ContentProvider {
    ...
    private static final HttpUrl BITCOINAVERAGE_JADEPAYBTC_URL = HttpUrl
            .parse("https://apiv2.bitcoinaverage.com/indices/crypto/ticker/JADEPAYBTC");
    ...
    public String getType(final Uri uri) {
     ...
     private Map<String, ExchangeRate> requestExchangeRates() {
         Double jadepayPerBTC = 0.0;
         try {
             jadepayPerBTC = requestExchangeRateOfJadepayInBTC_poloniex();

             if (jadepayPerBTC == null) {
                 Map<String, ExchangeRate> jadepayRates = requestExchangeRatesForJadepayInBTC();
                 jadepayPerBTC = Double.parseDouble(jadepayRates.get("JADEPAY").rate.fiat.toString()) / Double.parseDouble(jadepayRates.get("JADEPAY").rate.coin.toString());
             }
         }
         catch(Exception x)
         {
             log.warn("problem fetching exchange rates from " + BITCOINAVERAGE_JADEPAYBTC_URL, x);
             return null;
         }
                ...
                for (final Iterator<String> i = head.keys(); i.hasNext();) {
                    ...
                    if (currencyCode.startsWith("BTC")) {
                        ...
                        if (!fiatCurrencyCode.equals(MonetaryFormat.CODE_BTC)
                            ...
                            try {
                                Double _rate = jadepayPerBTC * Double.parseDouble(average);
    ...
    private Double requestExchangeRateOfJadepayInBTC_poloniex() {
        final Stopwatch watch = Stopwatch.createStarted();
    ...
    private Map<String, ExchangeRate> requestExchangeRatesForJadepayInBTC() {
        final Stopwatch watch = Stopwatch.createStarted();

        final Request.Builder request = new Request.Builder();
        request.url(BITCOINAVERAGE_JADEPAYBTC_URL);
        request.header("User-Agent", userAgent);
        ...
        try {
            ...
            if (response.isSuccessful()) {
                ...
                try {
                    final Fiat rate = parseFiatInexact("JADEPAY",  averages.getString("day"));
                    if (rate.signum() > 0)
                        rates.put("JADEPAY", new ExchangeRate(
                                new org.bitcoinj.utils.ExchangeRate(rate), BITCOINAVERAGE_SOURCE));
                } catch (final IllegalArgumentException x) {
                    log.warn("problem fetching {} exchange rate from {}: {}", "JADEPAY",
                            BITCOINAVERAGE_JADEPAYBTC_URL, x.getMessage());
                }

                watch.stop();
                log.info("fetched exchange rates from {}, {} chars, took {}", BITCOINAVERAGE_JADEPAYBTC_URL, content.length(),
                        watch);

                return rates;
            } else {
                log.warn("http status {} when fetching exchange rates from {}", response.code(), BITCOINAVERAGE_JADEPAYBTC_URL);
            }
        } catch (final Exception x) {
            log.warn("problem fetching exchange rates from " + BITCOINAVERAGE_JADEPAYBTC_URL, x);
        }
